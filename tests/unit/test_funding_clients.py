...
def _assert_public_results(
    ...
    assert page.items[0].status is KnownFundingTransactionStatus.COMPLETED
...
def _assert_public_requests(captured: list[httpx.Request]) -> None:
    ...
    assert all("POLY_SIGNATURE" not in request.headers for request in captured)
    ...
    assert dict(captured[4].url.params) == {"limit": "1"}
...
def test_all_clients_expose_final_funding_fetch_names_only() -> None:
    for client_type in (PublicClient, SecureClient, AsyncPublicClient, AsyncSecureClient):
        assert hasattr(client_type, "fetch_supported_funding_assets")
        assert hasattr(client_type, "fetch_funding_quote")
        assert not hasattr(client_type, "get_supported_funding_assets")
        assert not hasattr(client_type, "get_funding_quote")
...
def test_sync_public_funding_calls_use_bridge_transport_and_close_it() -> None:
    ...
    with PublicClient() as client:
        ...
        catalog = client.fetch_supported_funding_assets()
    ...
    _assert_public_results(deposit, withdrawal, catalog, quote, page)
...
def test_async_public_funding_calls_use_bridge_transport_and_close_it() -> None:
    ...
    async def run() -> tuple[
        ...
        async with AsyncPublicClient() as client:
            ...
            quote = await client.fetch_funding_quote(**_public_funding_args())
            page = await client.list_funding_transactions(
                address=_PUBLIC_WALLET_CHECKSUM,
                page_size=1,
            ).first_page()
        return deposit, withdrawal, catalog, quote, page, bridge_client

    deposit, withdrawal, catalog, quote, page, bridge_client = asyncio.run(run())
